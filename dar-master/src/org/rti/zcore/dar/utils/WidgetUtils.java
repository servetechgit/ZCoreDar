...
import org.rti.zcore.UserGroup;
 ...
 *         Date: Jul 12, 2005
 *         Time: 9:59:41 AM
 *         @deprecated - use the Zcore version.
 */
public class WidgetUtils {
     ...
     */
    public static String getOne(Connection conn, PageItem pageItem, String value, Long formId, Long encounterId, String widgetType, String currentFieldNameIdentifier, Long bridgeId) {
        ...
        if (encounterForm.getFormTypeId() == 6) {	// patient bridge table form
        	if (bridgeId == 0) {
        		updateRecord = "createRecord('" + inputType + "', "
                + formFieldId + " ," + pageItemId + " ," + formId + ", " + encounterId + " , '" + widgetType + "', '" + currentFieldNameIdentifier + "');";
        	} else {
        		updateRecord = "updateRecord('" + inputType + "', "
                + formFieldId + " ," + pageItemId + " ," + formId + ", " + encounterId + " , '" + widgetType + "', " + bridgeId + " , '" + currentFieldNameIdentifier + "');";
        	}
        } else {
        	updateRecord = "updateRecord('" + inputType + "', "
        ...
        if ((inputType.equals("emptyDate")) || (inputType.equals("birthdate")) || (inputType.equals("dateToday")) || (inputType.equals("dateOneMonthFuture"))) {
        ...
        } else if (inputType.equals("select") || inputType.equals("select-dwr") || inputType.equals("select-multitoggle")) {
        	...
        	List<DropdownItem> dropdownItems = null;
                if (pageItem.getInputType().equals("dropdown")) {
					try {
						dropdownItems = WidgetUtils.getList(conn, pageItem.getDropdownTable(), pageItem.getDropdownColumn(), pageItem.getDropdownConstraint(), pageItem.getDropdownOrderByClause(), DropdownItem.class);
					} catch (SQLException e) {
						log.error(e);
					} catch (ServletException e) {
						log.error(e);
        	...
        	renderSelect(visibleEnumIdTrigger1, sbuf, widgetName, formFieldId, updateRecord, dep1, encounterId, pageItem, visibleEnumIdTrigger2, dep2, value, dropdownItems, bridgeId, currentFieldNameIdentifier);
        	item = sbuf.toString();
    ...
    /**
     * For instances where identifier is a string, such as usernames, and the widgetType is a Chart.
     * Only returns a text field widget.
     * @param conn
     * @param pageItem
     * @param value
     * @param formId
     * @param identifier - such as username
     * @param widgetType - used for Chart widgettype only.
     * @return
     */
    public static String getOneStringIdentifier(Connection conn, PageItem pageItem, String value, Long formId, String identifier, String widgetType) {
		...
		widgetName = "inputWidget" + identifier + "." + String.valueOf(formFieldId);

    	if (formField.getStarSchemaName().equals("username")) {
			sbuf.append("You may not change username.\n");
			item = sbuf.toString();
	    	return item;
    	}
		updateRecord = "updateRecordChart('" + inputType + "', "
    		+ formFieldId + " ," + pageItemId + " ," + formId + ", '" + identifier + "' , '" + widgetType + "','');";

		if (formField.getStarSchemaName().equals("password")) {
			value = "";
		}

    	if (inputType.equals("text") || inputType.equals("text-dwr")) {
    		if ((pageItem.getSize() != null) && (pageItem.getSize().intValue() > 0)) {
    			sbuf.append("<input id=\"" + widgetName + "\" type=\"text\" name=\"field" + formFieldId + "\" size=\"" + pageItem.getSize() + "\" maxlength=\"" + pageItem.getMaxlength() + "\" value=\"" + value + "\"" + getStringEventHandler("onchange", updateRecord, identifier) + "/>\n");
    		} else {
    			sbuf.append("<input id=\"" + widgetName + "\" type=\"text\" name=\"field" + formFieldId + "\" size=\"20\" maxlength=\"255\" value=\"" + value + "\"" + getStringEventHandler("onchange", updateRecord, identifier) + "/>\n");
    		}
    		item = sbuf.toString();
    		} else {
    			sbuf.append("*tbd* " + inputType + "\n");
    ...
    /**
     * Creates input widget for reports
     * @param value
     * @param identifier
     * @param widgetType
     * @return
     */
    public static String getOneStringIdentifierNoForm(String value, String identifier, String widgetType) {
    	String item = null;
    	StringBuffer sbuf = new StringBuffer();
    	value = value.trim();
    	String widgetName = null;
    	String updateRecord = null;
    	widgetName = "inputWidget" + identifier;
    	updateRecord = "updateReportValue('" + identifier + "' , '" + widgetType + "','');";
    	sbuf.append("<input id=\"" + widgetName + "\" type=\"text\" name=\"field" + identifier + "\" size=\"6\" maxlength=\"255\" value=\"" + value + "\"" + getStringEventHandler("onchange", updateRecord, identifier) + "/>\n");
    	item = sbuf.toString();
    	return item;
    }

    /**
     * Renders html code for a select widget using a Set of FieldEnumerations
     * @param visibleEnumIdTrigger1
     * @param sbuf
     * @param widgetName
     * @param formFieldId
     * @param updateRecord
    ...
    /**
     * Renders html code for a select widget using a List of DropdownItems
     * @param visibleEnumIdTrigger1
     * @param sbuf
     * @param widgetName
     * @param formFieldId
     ...
     * @param dropdownItems
	 * @param bridgeId - id of the record from the patient bridge table.
     * @param currentFieldNameIdentifier - String value that identified where to render the value.
     */
    private static void renderSelect(String visibleEnumIdTrigger1, StringBuffer sbuf, String widgetName, Long formFieldId, String updateRecord, String dep1, Long encounterId, PageItem pageItem, String visibleEnumIdTrigger2, String dep2, String value, List dropdownItems, Long bridgeId, String currentFieldNameIdentifier) {
    	if ((!visibleEnumIdTrigger1.equals("0")) && (new Long(visibleEnumIdTrigger1).intValue() > 0) && (formFieldId.intValue() != 1677))
    	{
    		sbuf.append("<select id=\"" + widgetName + "\" name=\"field" + formFieldId + "\"" + getEventHandler("onchange", updateRecord, dep1, encounterId, pageItem) + ">\n");
    	} else if ((!visibleEnumIdTrigger1.equals("0")) && new Long(visibleEnumIdTrigger2).intValue() > 0) {
    		sbuf.append("<select id=\"" + widgetName + "\" name=\"field" + formFieldId + "\"" + getEventHandler("onchange", dep2, updateRecord, encounterId, pageItem) + ">\n");
    	} else {
    		sbuf.append("<select id=\"" + widgetName + "\" name=\"field" + formFieldId + "\"" + getEventHandler("onchange", updateRecord, encounterId) + ">\n");
    	...
    	for (Iterator iterator = dropdownItems.iterator(); iterator.hasNext();) {
    		DropdownItem dropdownItem = (DropdownItem) iterator.next();
    		// replace html escape tages
    		// todo: should we handle this further up the chain?
    		String valueFixed = value.replaceAll("&lt;", "<");
    			if (valueFixed.equals(dropdownItem.getDropdownValue())) {
    				sbuf.append("   <option value=\"" + dropdownItem.getDropdownId() + "\" selected>" +
    						dropdownItem.getDropdownValue() + "</option>\n");
    			} else {
    				sbuf.append("   <option value=\"" + dropdownItem.getDropdownId() + "\">" +
    						dropdownItem.getDropdownValue() + "</option>\n");
    			}
    	}
    	sbuf.append("</select>\n");
    ...
    /**
     * For instances where the identifier is a String, such as usernames
     * @param eventName
     * @param update
     * @param identifier - such as a username
     * @return
     */
    private static String getStringEventHandler(String eventName, String update, String identifier) {
    ...
    /**
     * For pageItems in a patient bridge table
     * @param eventName
     * @param update
     * @param encounterId
     * @param bridgeId
     * @return
     */
    private static String getEventHandler(String eventName, String update, Long encounterId, Long bridgeId) {
    ...
    public static String getGroups(Connection conn, String username) throws SQLException, ObjectNotFoundException, ServletException {
        String item = null;
        StringBuffer sbuf = new StringBuffer();
        sbuf.append("<select id=\"groupsDropdown\" onchange=\"modifyUserGroup('" + username + "')\">\n");
     ...
     * Does a little jiggery pokery on the tablename to help out in situations where you're joining multiple tables.
     * @param conn
     * @param tableName
     * @param columnName
     * @param constraint
     ...
     */
    public static List<DropdownItem> getList(Connection conn, String tableName, String columnName, String constraint, String orderBy, Class clazz) throws SQLException, ServletException {
        List list = new ArrayList();
        ...
        if (constraint != null) {
        	if (orderBy != null) {
        		sql = "select " + firstTable + ".id AS dropdownId, " + columnName + " AS dropdownValue " +
                "from " + tableName + " " + constraint + " " + orderBy;
        	} else {
        		sql = "select " + firstTable + ".id AS dropdownId, " + columnName + " AS dropdownValue " +
                "from " + tableName + " " + constraint;
        	}

        } else {
        	sql = "select id AS dropdownId, " + columnName + " AS dropdownValue " +
            "from " + tableName + "\n";
        }
        list = DatabaseUtils.getList(conn, clazz, sql, values);
        return list;
    }

    /**
     * Fetches item from Dynamic dropdown table
     * @param conn
     * @param tableName
     * @param columnName
     * @param value
     ...
     */
    public static DropdownItem getDropdownItem(Connection conn, String tableName, String columnName, Integer value, String constraint) throws SQLException, ServletException, ObjectNotFoundException {
		...
		/*sql = "select id AS dropdownId, " + columnName + " AS dropdownValue " +
		"from " + tableName + " " +
		"where " + firstTable + ".id=" + value;*/

		sql = sql.concat("where " + firstTable + ".id=" + value);
