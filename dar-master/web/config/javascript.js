	...
	if(document.all) {
		...
		*/
		...
		else
		{
			...
			// calendar widget it 136 px.
			// kudos: http://www.quirksmode.org/js/events_properties.html
			var posy = event.clientY + document.body.scrollTop
			+ document.documentElement.scrollTop;
			//alert("posy:" + posy + " event.clientY: " + event.clientY + " document.body.scrollTop: " + document.body.scrollTop + " document.documentElement.scrollTop: " + document.documentElement.scrollTop);
			if (posy < 200) {
				document.all[divcal].style.top = posy;
			} else {
				document.all[divcal].style.top = posy-200;
			}
		...
		//alert("posy:" + posy + " event.clientY: " + event.clientY + " document.body.scrollTop: " + document.body.scrollTop + " document.documentElement.scrollTop: " + document.documentElement.scrollTop);
		if (posy < 200) {
			document.getElementById(divcal).style.top = posy;
		} else {
			document.getElementById(divcal).style.top = posy-200;
		}
...
function dtemaj(jour, mois, annee, theDiv){
    ...
    var dbDate = padNumber(jour, 2);
    var dbMonth = padNumber(mois, 2);
    var dbValue = annee.toString() + "-" + dbMonth.toString() + "-" + dbDate.toString();
    inputWidget.value = dbValue.toString();
	if (calformelement2 !=null) {
	    document.forms[calformname].elements[calformelement2].value = dbValue;
	}
	displayname = "span"+calformelement;
    display = eval(getLabel(displayname));
    display.innerHTML = formatDate(jour, mois, annee);
    // alert("inputWidget.value: " + inputWidget.value + " calformelement: " + calformelement + " dbValue: " + dbValue);
...
function formatDate(day, month, year) {
	...
	var previousPattern;
	var patternLength = 0;
	// alert("calpattern: "+calpattern + " month: " + month);
		...
		while (pos <= calpattern.length) {
			...
			if (pattern != previousPattern) {
				switch (previousPattern) {
					case 'd':
						paddedDay = padNumber(day, patternLength);
						date += paddedDay;
						break;
					case 'M':
						if (calpattern == "dd MMM yy") {
							//alert("month: " + month);
							switch (month) {
								case '1':
									paddedMonth = "Jan";
