...
<c:forEach var="pageItem" begin="0" items="${form.pageItems}" varStatus="lineItem">
    ...
    <c:choose>
        <c:when test="${pageItem.inputType == 'currentMedicine'}">
            ...
            <html:hidden property="${field.identifier}" styleId="item${lineItem.index}"/>
        </c:when>
    </c:choose>
</c:forEach>
            ...
            <table>
                ...
                <tr>
                    ...
                    <td valign="top">
                        ...
                        <c:choose>
                        <c:when test="${pageItem.inputType=='currentMedicine'}">
                        <bean:define id="thisDrug" name="${encounterForm.classname}" property="${field.identifier}"/>
...
<%--
            <html:hidden property="${field.identifier}" styleId="_fieldOrder"/>
--%>
        </td>
    </tr>
    ...
    <tr>
        ...
        <html:text size="40" maxlength="255" property="${field.identifier}"/> ${field.units}
        </c:when>
        </c:choose>
        </c:if>
        </c:forEach>
