...
<c:if test="${encounterForm.id == 2}">
    ...
    <c:url value="patientHome.do" var="url"><c:param name="patientId" value="${zeprs_session.sessionPatient.id}"/>
        <c:choose>
            <c:when test="${! empty currentPregId}"><c:param name="eventId" value="${currentPregId}"/></c:when>
            <c:otherwise>
    ...
    <c:if test="${currentPregId != zeprs_session.sessionPatient.currentPregnancyId}">
        <br><a href='<c:out value="/${appName}/${url}"/>'>Return to Current Pregnancy</a>
    </c:if>
</c:if>
...
<c:choose>
    ...
    <c:otherwise>
        <c:forEach items="${chartItems}" var="encounter" varStatus="item">
            ...
            <td id="${field.identifier}Cell${pos}" valign="middle" class="${tdClass}">
                <c:choose>
                    <c:when test="${! empty encounter}">
            ...
            <c:set var="pos" value="0"/>
            <td id="${field.identifier}Cell${pos}" valign="middle" class="highlightRow"><zeprs:date_visit_only pageItem="${pageItem}" pos="${pos}"/></td>
            </tr>
            <c:set var="pos" value="${encounter.id}"/>
            </c:if>
            <c:set var="tdClass" value="enhancedtabletighterCell"/>
        ...
        <c:forEach items="${chartItems}" var="encounter" varStatus="item">
            ...
            <td id="${field.identifier}Cell${pos}" valign="middle" class="${tdClass}" ondblclick="callMetaDataWidget('SiteId.${encounter.id}', ${encounterForm.id}, '${encounter.id}')">
                <c:choose>
                    <c:when test="${! empty encounter}">
                    ...
                    <c:when test="${! empty encounter}">
                        ...
                        <logic:empty name="encounter" property="dateVisit">
                         ...
                         <logic:present role="DELETE_ARCHIVE_PATIENT_RECORDS">
                            ...
                            <br/>
                            <a title="Delete record?" href='<c:out value="/${appName}/${delUrl}"/>'onclick="return confirm('Caution: Are you sure you want to delete this record from ZEPRS?');self.close();">X</a>
                        </logic:present>
                    </c:when>
                </c:choose>
                </c:if>
            ...
            <c:if test="${item.last && dataEntry == 1}">
            <td id="${field.identifier}Cell${pos}" valign="middle" class="highlightRow">&nbsp;</td>
            </tr>
            <c:set var="pos" value="${encounter.id}"/>
            </c:if>
            <c:set var="tdClass" value="enhancedtabletighterCell"/>
                ...
                <c:choose>
                    <c:when test="${encounterForm.id == '55'}">
                        <c:if test="${lineInfo.first}">
                            <td id="${field.identifier}Cell${pos}" valign="middle" class="highlightRow" rowspan="${fn:length(encounterForm.pageItems)}">
                    ...
                    <c:otherwise>
                    	<c:choose>
    						<c:when test="${pageItem.visible=='false'}">
    							<td id="${field.identifier}Cell${pos}" valign="middle" class="highlightRow" style="visibility:hidden" >
    						</c:when>
    						<c:otherwise>
    							<td id="${field.identifier}Cell${pos}" valign="middle" class="highlightRow">
    						</c:otherwise>
    					</c:choose>
...
<c:otherwise>
    ...
    <c:choose><%--Now show the page items--%>
            ...
            <c:choose>
               ...
               <c:when test='${pageItem.inputType=="checkbox_dwr"}'>
                    <c:if test="${field.id == '1655'}">
                        <logic:present name="${encounterForm.classname}" property="field1655">
                        <bean:define id="thisValue" name="${encounterForm.classname}" property="field${field.id}"/>
                        </logic:present>
                     </c:if>
        ...
        <c:when test='${pageItem.inputType=="ega_pregnancyDating"}'>
            <zeprs:ega_pregnancyDating pageItem="${pageItem}" pos="${pos}" remoteClass="${remoteClass}" classname="${classname}" propertyName="${pageItem.form_field.starSchemaName}" patientId="${zeprs_session.sessionPatient.id}" eventId="${zeprs_session.sessionPatient.currentEventId}" user = "${user}" siteId="${siteId}" formId="${pageItem.form.id}"  value="${egaToday}"/>
        </c:when>
