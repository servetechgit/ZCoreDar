...
<c:when test="${flowId==2}"><h2>Dispensing${labelEnd}</h2></c:when>
<c:when test="${flowId==3}"><h2>Health${labelEnd}</h2></c:when>
<c:when test="${flowId==4}"><h2>ART Regimen${labelEnd}</h2></c:when>
...
<tbody>
 ...
 <c:if test="${flowId == 2}">
 <tr>
    <td><html:link action="/PatientItem/new.do" paramId="patientId" paramName="patientId" styleClass="noBorder"><img src="/${appName}/images/checkbox.gif" height="15px" width="15px"/></html:link></td>
    <td><html:link action="/PatientItem/new.do" paramId="patientId" paramName="patientId"><bean:message key="PatientItem.label" bundle="PatientItemMessages" /></html:link></td>
    <td colspan="5" class="error">${doneDialogue}</td>
</tr>
 </c:if>
<c:set var="doneDialogue" value="Not Done"/>
<logic:iterate id="task" name="tasklist" property="taskList">
<c:choose>
    <c:when test="${empty zeprs_session.sessionPatient.dateEventEnd}">
         <c:choose>
        ...
        <c:choose>
	        <c:when test="${task.active=='true'}">
					<c:choose>
					<c:when test="${task.formTypeId == 8}">
			            <tr>
			                <td><html:link action="/${task.className}/new.do" paramId="patientId" paramName="task" paramProperty="patientId" styleClass="noBorder"><img src="/${appName}/images/checkbox.gif" height="15px" width="15px"/></html:link></td>
			                <td><html:link action="/${task.className}/new.do" paramId="patientId" paramName="task" paramProperty="patientId"><bean:write name="task" property="label"/></html:link></td>
							<c:choose>
							...
							<td>${task.siteName}</td>
							<td><fmt:formatDate type="both" pattern="dd MMM yy  HH:mm" value="${task.auditInfo.lastModified}" /></td>
							</c:when>
							<c:otherwise>
					...
					<c:otherwise>
			            ...
			            <tr>
			                <td><html:link action="/${task.className}/new.do" paramId="patientId" paramName="task" paramProperty="patientId" styleClass="noBorder"><img src="/${appName}/images/checkbox.gif" height="15px" width="15px"/></html:link></td>
			                <td><html:link action="/${task.className}/new.do" paramId="patientId" paramName="task" paramProperty="patientId"><bean:message key="${task.className}.label" bundle="${task.className}Messages" /></html:link></td>
			                <td colspan="5" class="error">${doneDialogue}</td>
			            </tr>
			            </c:if>
					</c:otherwise>
	        ...
	        <c:otherwise>
		        <%--tasks that are not active - forms that have encounters --%>
		        <c:if test="${!empty task.encounterId}">
		        	<c:choose>
			            <c:when test="${task.formId == 79}">
			                <tr>
			                    <td><html:link action="/partograph.do" paramId="patientId" paramName="task" paramProperty="patientId" styleClass="noBorder"><img src="/${appName}/images/checkbox.gif" height="15px" width="15px"/></html:link></td>
			            ...
			            <c:when test="${task.formId == 23}">
			             <tr>
			                <td><c:url value="viewEncounter.do" var="url"><c:param name="id" value="${task.encounterId}"/><c:param name="class" value="${task.className}"/></c:url>
			                        <a href='<c:out value="/${appName}/${url}"/>' class='noBorder'><img src="/${appName}/images/checkboxChecked.gif" height="15px" width="15px"/></a></td>
			                ...
			                <a href='<c:out value="/${appName}/${url}"/>'><fmt:formatDate type="both" pattern="dd MMM yy  HH:mm" value="${task.auditInfo.lastModified}" /></a>
			                </td>
			                </tr>
			            </c:when>
			            ...
			            <c:otherwise>
			                ...
			                <td>${task.auditInfo.lastModifiedBy.lastName}, ${task.auditInfo.lastModifiedBy.firstName}</td>
			                <td>${task.siteName}</td>
			                <td><fmt:formatDate value="${task.dateVisit}" pattern="dd MMM yy"/></td>
			                <td>
			                <c:url value="viewEncounter.do" var="url">
			                    <c:param name="id" value="${task.encounterId}"/>
			                    <c:param name="patientId" value="${task.patientId}"/>
        ...
        <c:choose>
           <c:when test="${!empty task.encounterId}">
               <c:choose>
                    ...
                    <td>${task.siteName}</td>
                    <td><c:url value="viewEncounter.do" var="url"><c:param name="id" value="${task.encounterId}"/><c:param name="class" value="${task.className}"/></c:url>
                            <a href='<c:out value="/${appName}/${url}"/>'><fmt:formatDate type="both" pattern="dd MMM yy  HH:mm" value="${task.auditInfo.lastModified}" /></a></td>
                    </tr>
                    </c:otherwise>
                </c:choose>
           </c:when>
            <c:otherwise>
                <c:choose>
                    ...
                    <tr>
                        <td><html:link action="/previousPregnancies.do" styleClass="noBorder" paramId="patientId" paramName="task" paramProperty="patientId"><img src="/${appName}/images/checkboxChecked.gif" height="15px" width="15px"/></html:link></td>
                        <td><bean:write name="task" property="label"/></td>
                        <c:choose>
                            <c:when test="${! empty task.auditInfo.lastModified}">
                            <td>Done</td>
                            <td>${task.auditInfo.lastModifiedBy.lastName}, ${task.auditInfo.lastModifiedBy.firstName}</td>
                            <td>${task.siteName}</td>
                            <td><html:link action="/previousPregnancies.do" paramId="patientId" paramName="task" paramProperty="patientId"><fmt:formatDate type="both" pattern="dd MMM yy  HH:mm" value="${task.auditInfo.lastModified}"/></html:link></td>
                            </c:when>
                            <c:when test="${! empty zeprs_session.sessionPatient.noPreviousPregnancies && empty zeprs_session.sessionPatient.dateEventEnd}">
                            ...
                            <td colspan="4" class="error">No Previous Pregnancies
                                <c:url value="patientHome.do" var="url">
                                    ...
                                    <c:param name="eventId" value="0"/>
                                </c:url>
                                <p><a href='<c:out value="/${appName}/${url}"/>'>Return to Current Pregnancy</a></p>
                            </td>
                            </c:when>
                            <c:otherwise>
                            <td colspan="4" class="error">Not Done</td>
...
<table class="enhancedtable">
    ...
    <logic:iterate id="thisproblem" name="longTermProblems">
        ...
        <c:choose>
            <c:when test="${thisproblem.class.name=='org.rti.zcore.rules.impl.EncounterOutcome'}">
            ...
            <c:when test="${thisproblem.class.name=='org.cidrz.webapp.dynasite.rules.impl.ReferralOutcome'}">
                <c:choose>
                    <c:when test="${problemId==thisproblem.id && outcome==1}">
                        ...
                        <c:set var="currclass" value="${thisproblem.class.name}" scope="request"/>
                        <tr class="currentProblem">
                            <td><a href='<c:out value="/${appName}/${editUrl}"/>'>${thisproblem.reason}</a> <a
                                    href='<c:out value="/${appName}/${editUrl}"/>'>(e)</a></td>
                            <td width="50px"><fmt:formatDate type="both" pattern="dd MMM yy"
                                                             value="${thisproblem.lastModified}"/></td>
                        </tr>
                    </c:when>
