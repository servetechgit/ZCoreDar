...
<template:put name='title' direct='true'>Appointment Register for ${register.siteName} ${beginDate} - ${endDate}</template:put>
<template:put name='header' direct='true'>Appointment Register for ${register.siteName} ${beginDate} - ${endDate}</template:put>
<template:put name='content' direct='true'>
<div id="forms">
<h1>Appointment Register for ${register.siteName} ${beginDate} - ${endDate}</h1>
...
<tr>
    ...
    <td align="center" class="small">
     ...
     <td align="center" class="small">
         <c:choose>
            <c:when test="${! empty patient.appointmentDate}"> <fmt:formatDate value="${patient.appointmentDate}" pattern="${dateFormatLong}"/></c:when>
            <c:otherwise>&nbsp;</c:otherwise>
        </c:choose>
     </td>
</tr>
