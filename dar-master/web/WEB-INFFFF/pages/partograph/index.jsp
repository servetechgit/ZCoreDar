...
<script language="JavaScript" type='text/javascript' src='/${appName}/dwr/interface/Partograph.js;jsessionid=${pageContext.request.session.id}'></script>
<script language="JavaScript" TYPE="text/javascript" src="/${appName}/js/partograph.js;jsessionid=${pageContext.request.session.id}"></script>
    ...
    <c:when test="${subject.id != null}">
        <h2>Partograph
            <logic:present role="DELETE_ARCHIVE_PATIENT_RECORDS">
             ...
             &nbsp;(<a href='<c:out value="/${appName}/${delUrl}"/>'onclick="return confirm('Caution: Are you sure you want to delete this record from ZEPRS?');self.close();" title="Delete this record.">X</a>)
            </logic:present>
        </h2>
    </c:when>
    <c:otherwise>
...
<table width="100%">
    <tr>
        ...
        <c:choose>
            <c:when test="${partoPage == 1}">
            Hours 1-8 | <a href='<c:out value="/${appName}/${parto2}"/>'>Hours 9-16</a> | <a href='<c:out value="/${appName}/${parto3}"/>'>Hours 17-24</a>
            </c:when>
            <c:when test="${partoPage == 2}">
            <a href='<c:out value="/${appName}/${parto1}"/>'>Hours 1-8</a> | Hours 9-16 | <a href='<c:out value="/${appName}/${parto3}"/>'>Hours 17-24</a>
            </c:when>
            <c:when test="${partoPage == 3}">
            <a href='<c:out value="/${appName}/${parto1}"/>'>Hours 1-8</a> | <a href='<c:out value="/${appName}/${parto2}"/>'>Hours 9-16</a> | Hours 17-24
            </c:when>
            <c:otherwise>
            <a href='<c:out value="/${appName}/${parto1}"/>'>Hours 1-8</a> | <a href='<c:out value="/${appName}/${parto2}"/>'>Hours 9-16</a> | <a href='<c:out value="/${appName}/${parto3}"/>'>Hours 17-24</a>
            </c:otherwise>
        </c:choose>
        </td>
    </tr>
