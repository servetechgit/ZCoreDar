...
<c:if test="${row =='200'}">
<td rowspan="15" class="partoLabel"><p>Diast.: <img src="/${appName}/images/arrow_red_down.gif"></p>
<p>
...
<c:choose>
    <c:when test="${! empty bloodPressure}">
        <c:forEach var="item" varStatus="loop"  begin="${startRow}" end="${stopRow}">
            ...
            <logic:equal value="${row}" name="bloodPressure" property="diastolic${pos}">
                <td valign="bottom" onclick="insertDiastolic(${row}, ${pos},${zeprs_session.sessionPatient.id}, ${zeprs_session.sessionPatient.currentEventId},'${user}',${siteId} )" id="diastolicCell${row}.${pos}">
                    <div id="diastolicCellDiv${row}.${pos}">
                        <img alt="diastolicImg${row}.${pos}" id="diastolicImg${row}.${pos}" src="/${appName}/images/arrow_red_down.gif">
                        <c:if test="${! empty pulse}">
                            <logic:equal value="${row}" name="pulse" property="pulse${pos}">
                                <img alt="pulseImgBD${row}.${pos}" id="pulseImgBD${row}.${pos}" src="/${appName}/images/dot.gif">
                            </logic:equal>
                        </c:if>
                        <logic:equal value="${row}" name="bloodPressure" property="systolic${pos}">
                            <img alt="systolicImg${row}.${pos}" id="systolicImgBD${row}.${pos}" src="/${appName}/images/arrow_up.gif">
                        </logic:equal>
                    </div>
                </td>
            </logic:equal>
            <logic:notEqual value="${row}" name="bloodPressure" property="diastolic${pos}">
            <td valign="top" onclick="insertDiastolic(${row}, ${pos},${zeprs_session.sessionPatient.id}, ${zeprs_session.sessionPatient.currentEventId},'${user}',${siteId} )" id="diastolicCell${row}.${pos}">
                <div id="diastolicCellDiv${row}.${pos}">
                    <c:if test="${! empty pulse}">
                        <logic:equal value="${row}" name="pulse" property="pulse${pos}">
                            <img alt="pulseImgBD${row}.${pos}" id="pulseImgBD${row}.${pos}" src="/${appName}/images/dot.gif">
    ...
    <c:otherwise>
        <c:forEach var="item" varStatus="loop"  begin="${startRow}" end="${stopRow}" >
            <c:set var="pos" value="${loop.index}"/>
            <td valign="top" onclick="insertDiastolic(${row}, ${loop.index},${zeprs_session.sessionPatient.id}, ${zeprs_session.sessionPatient.currentEventId},'${user}',${siteId} )" id="diastolicCell${row}.${loop.index}">&nbsp;
                <div id="diastolicCellDiv${row}.${pos}">
                    <c:if test="${! empty pulse}">
                        <logic:equal value="${row}" name="pulse" property="pulse${pos}">
                            <img alt="pulseImgBD${row}.${pos}" id="pulseImgBD${row}.${pos}" src="/${appName}/images/dot.gif">
                   ...
                   <%-- <logic:equal value="${row}" name="bloodPressure" property="systolic${pos}">
                        <img alt="systolicImg${row}.${pos}" id="systolicImg${row}.${pos}" src="/${appName}/images/arrow_up.gif">
                    </logic:equal>--%>
                </div>
            </td>
        </c:forEach>
