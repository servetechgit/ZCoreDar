...
<c:if test="${row =='200'}">
<td rowspan="15" class="partoLabel"><p>Pulse: <img src="/${appName}/images/dot_red.gif"></p>
<p>
...
<c:choose>
    <c:when test="${! empty pulse}">
        <c:forEach var="item" varStatus="loop"  begin="${startRow}" end="${stopRow}">
            ...
            <logic:equal value="${row}" name="pulse" property="pulse${pos}">
                <td valign="top" onclick="insertPulse(${row}, ${pos},${zeprs_session.sessionPatient.id}, ${zeprs_session.sessionPatient.currentEventId},'${user}',${siteId} )" id="pulseCell${row}.${pos}">
                    <div id="pulseCellDiv${row}.${pos}">
                        <img alt="pulseImg${row}.${pos}" id="pulseImg${row}.${pos}"src="/${appName}/images/dot_red.gif">
                        <c:if test="${! empty bloodPressure}">
                            <logic:equal value="${row}" name="bloodPressure" property="systolic${pos}">
                                <img alt="systolicImg${row}.${pos}" id="systolicImgBP${row}.${pos}" src="/${appName}/images/arrow_up.gif">
                            </logic:equal>
                            <logic:equal value="${row}" name="bloodPressure" property="diastolic${pos}">
                                <img alt="diastolicImg${row}.${pos}" id="diastolicImgBP${row}.${pos}" src="/${appName}/images/arrow_down.gif">
                            </logic:equal>
                        </c:if>
                    </div>
                    </td>
    ...
    <c:otherwise>
        <c:forEach var="item" varStatus="loop"  begin="${startRow}" end="${stopRow}">
            <c:set var="pos" value="${loop.index}"/>
            <td valign="top" onclick="insertPulse(${row}, ${loop.index},${zeprs_session.sessionPatient.id}, ${zeprs_session.sessionPatient.currentEventId},'${user}',${siteId} )" id="pulseCell${row}.${loop.index}">
                <div id="pulseCellDiv${row}.${pos}">&nbsp;
                    <c:if test="${! empty bloodPressure}">
                        <logic:equal value="${row}" name="bloodPressure" property="systolic${pos}">
                            <img alt="systolicImg${row}.${pos}" id="systolicImgBP${row}.${pos}" src="/${appName}/images/arrow_up.gif">
...
var replyPulseOLD = function(data) {
    ...
    //item.innerHTML = '<img src="/${appName}/images/dot.gif">';
    ...
    sitem.innerHTML = '<img src="/${appName}/images/dot.gif">';
    ditem.innerHTML = '<img src="/${appName}/images/dot.gif">';
    pitem.innerHTML = '<img src="/${appName}/images/dot_red.gif">';
}
