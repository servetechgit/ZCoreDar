    ...
    <c:choose>
        ...
        <c:forEach var="pageItem" begin="0" items="${encounterForm.pageItems}" varStatus="lineInfo">
            ...
            <c:set var="currentField" value="${field.id}" scope="request"/>
                <c:choose>
                    <c:when test="${! empty encounter}">
            <td id="${field.identifier}Cell${pos}" onclick="justReveal('field${field.id}Field${pos}')" valign="middle" class="enhancedtabletighterCell">
                    </c:when>
                    <c:otherwise>
            <td id="${field.identifier}Cell${pos}" valign="middle" class="enhancedtabletighterCell">
                    </c:otherwise>
                </c:choose>
