    ...
    <c:if test='${pageItem.form_field.enabled ==true && pageItem.form_field.type != "Display"}'>
	    <c:if test='${pageItem.inputType != "hidden-no-listing"}'>
			...
			<c:when test="${field.type == 'Date'}">
				<th id="${field.identifier}Cell${pos}" valign="middle" class="enhancedtabletighterCell" width="60px;"><strong><bean:message key="${encounterForm.classname}.${field.identifier}" bundle="${encounterForm.classname}Messages" /></strong></th>
			</c:when>
			<c:otherwise>
...
<c:otherwise>
    ...
    <c:forEach items="${chartItems}" var="encounter" varStatus="item">
        <tr>
            ...
            <c:choose>
               	<c:when test="${encounterForm.id == 170}"><c:set var="currentEncounterId" value="${encounter.username}" scope="request"/></c:when>
            ...
            <c:if test='${pageItem.form_field.enabled ==true && pageItem.form_field.type != "Display" && pageItem.inputType != "multiselect_enum" && pageItem.inputType != "hidden-no-listing"}'>
                ...
                <c:set var="currentField" value="${field.id}" scope="request"/>
                <c:choose>
                <c:when test="${(encounterForm.id == 161) && ((pageItem.id == 4508 ) || (pageItem.id == 4503) || (pageItem.id == 4519) || (pageItem.id == 4520) || (pageItem.id == 4517))}">
                	<td id="cell${currentEncounterId}.${field.id}" valign="middle" class="${tdClass}">
                </c:when>
                <c:when test="${disableForm == 1}">
                	<td id="cell${currentEncounterId}.${field.id}" valign="middle" class="${tdClass}">
                </c:when>
                <c:otherwise>
            ...
            <c:choose>
	            ...
	            <logic:present role="DELETE_ARCHIVE_PATIENT_RECORDS">
	                ...
	                <td valign="middle" class="${tdClass}"><a href='<c:out value="/${appName}/${delUrl}"/>' onclick="return confirm('Caution: Are you sure you want to delete this record from ZEPRS?');self.close();">X</a></td>
	            </logic:present>
	            </c:otherwise>
            </c:choose>
        </tr>
