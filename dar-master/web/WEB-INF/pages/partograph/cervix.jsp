...
<td rowspan="13" class="partoLabel">
    <div>
        <p>Cervix <img src="/${appName}/images/dot_red.gif"><br/>
        (cm)</p>
        <p>
            <a href="#" onclick="toggleDescent()">Switch to Desc. Plot</a> <img src="/${appName}/images/triangle.gif">
        </p>
    </div>
</td>
</c:if>
...
<c:choose>
    <c:when test="${! empty cervix}">
        <c:forEach var="item" varStatus="loop"  begin="${startRowHour}" end="${stopRowHour}">
            <c:set var="pos" value="${loop.index}"/>
            <logic:equal value="${row}" name="cervix" property="cervix${pos}">
                <td align="center" colspan="2" onclick="insertCervix(${row}, ${pos},${zeprs_session.sessionPatient.id}, ${zeprs_session.sessionPatient.currentEventId},'${user}',${siteId} )" id="cervixCell${row}.${pos}" width="14">
                    <div id="cervixCellDiv${row}.${pos}">
                        <img alt="CervixImg${row}.${pos}" id="CervixImg${row}.${pos}" src="/${appName}/images/dot_red.gif">
                        <script type='text/javascript'>
                        ...
                        <logic:equal value="${row}" name="descent" property="descent${pos}">
                            <img id="DescentImgB${row}.${pos}" src="/${appName}/images/triangle.gif"></logic:equal>
                        </c:if>
                        </div></td>
            </logic:equal>
    ...
    <c:otherwise>
        <c:forEach var="item" varStatus="loop"  begin="${startRowHour}" end="${stopRowHour}">
            <c:set var="pos" value="${loop.index}"/>
            <td align="center" colspan="2" onclick="insertCervix(${row}, ${loop.index},${zeprs_session.sessionPatient.id}, ${zeprs_session.sessionPatient.currentEventId},'${user}',${siteId} )" id="cervixCell${row}.${loop.index}" width="14">
                <div id="cervixCellDiv${row}.${pos}">
                     <c:if test="${! empty descent}">
                        <logic:equal value="${row}" name="descent" property="descent${pos}">
                            <img id="DescentImgB${row}.${pos}" src="/${appName}/images/triangle.gif">
                        </logic:equal>
                     </c:if>
                </div>
            </td>
...
function createActionLineValues(row, column) {
    Partograph.updateActionLineValues(null, row, column, ${zeprs_session.sessionPatient.id}, ${zeprs_session.sessionPatient.currentEventId}, '${user}');
}
function updateActionLineValues(row, column) {
    Partograph.updateActionLineValues(replyUpdateActionLines, row, column, ${zeprs_session.sessionPatient.id}, ${zeprs_session.sessionPatient.currentEventId}, '${user}');
}
</script>
