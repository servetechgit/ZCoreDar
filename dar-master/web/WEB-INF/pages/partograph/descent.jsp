    ...
    <tr id="descentRow${iRow.index}" style="display:none; border:none">
        <c:if test="${row =='10'}">
            <td rowspan="13" class="partoLabel">
                <div>
                    <p>Desc. Plot <img src="/${appName}/images/triangle_red.gif"></p>
                    <p>
                        <a href="#" onclick="toggleDescent()">Switch to Cervix</a> <img src="/${appName}/images/dot.gif">
                    </p>
                </div>
            </td>
        </c:if>
        ...
        <c:choose>
                ...
                <c:forEach var="item" varStatus="loop" begin="${startRowHour}" end="${stopRowHour}">
                    ...
                    <logic:equal value="${row}" name="descent" property="descent${pos}">
                        <td align="center" colspan="2"
                            onclick="insertDescent(${row}, ${pos},${zeprs_session.sessionPatient.id}, ${zeprs_session.sessionPatient.currentEventId},'${user}',${siteId} )"
                            id="descentCell${row}.${pos}">
                        <div id="descentCellDiv${row}.${pos}">
                            <img id="DescentImg${row}.${pos}" src="/${appName}/images/triangle_red.gif">
                            <c:if test="${! empty cervix}">
                                <logic:equal value="${row}" name="cervix" property="cervix${pos}">
                                    <img  id="CervixImgB${row}.${pos}" src="/${appName}/images/dot.gif">
                                </logic:equal>
                            </c:if>
                        </div></td>
                    </logic:equal>
            ...
            <c:otherwise>
                <c:forEach var="item" varStatus="loop" begin="${startRowHour}" end="${stopRowHour}">
                    <c:set var="pos" value="${loop.index}"/>
                    <td align="center" colspan="2"
                        onclick="insertDescent(${row}, ${loop.index},${zeprs_session.sessionPatient.id}, ${zeprs_session.sessionPatient.currentEventId},'${user}',${siteId} )"
