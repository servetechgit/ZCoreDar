...
<c:when test="${! empty value}">
    ...
    <span id="spandateVisit${pos}" onclick="justReveal('correctDateVisit${pos}'); showCalendar('${yearnow}','${monthnow}','${datenow}','dd MMM yy','form${pos}','dateVisit${pos}',event,${lastTwoYears},${twoYears});">${niceDate}</span>
    ...
    <span id="correctDateVisit${pos}" style="display:none;">
        ...
        <input type="button" name="_add" value="Change" onclick="insertInputField('${remoteClass}', ${scriptName}, '${classname}','dateVisit', ${pos},'dateVisit${pos}',${patientId}, ${eventId},'${user}',${siteId})">
    </span>
</c:when>
<c:otherwise>
