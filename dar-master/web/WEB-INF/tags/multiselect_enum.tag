...
<table>
    ...
    <tr>
        <td valign="top"> <select size="10" name="_allitems${field.id}" id="_allitems${field.id}">
        ...
        <td valign="top">
            <select size="10" name="_items${field.id}" id="_items${field.id}">
            ...
            <%--<c:forEach var="fpageItem" begin="0" items="${pageItems}" varStatus="lineItem">
                ...
                <bean:define id="thisItem" name="${encounterForm.classname}" property="${field.identifier}"/>
                <logic:iterate id="item" name="field" property="enumerations">
