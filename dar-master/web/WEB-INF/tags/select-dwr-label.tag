...
<bean:message key="${encounterForm.classname}.${field.identifier}" bundle="${encounterForm.classname}Messages" />: <c:if test='${field.required}'><span class="asterix">*</span> </c:if>
<c:choose>
    <c:when test="${! empty value}">
    <span id="${field.identifier}Results${pos}">${value}</span>
    <select name="${field.identifier}${pos}" id="${field.identifier}" onchange="insertSelectField('${remoteClass}', ${scriptName}, '${classname}','${field.identifier}', ${pos},'${field.identifier}Field${pos}',${patientId}, ${eventId},'${user}',${siteId},${formId})" style="display:none;">
        <option value=""> -- </option>
    ...
    <c:otherwise>
        ...
        <select name="${field.identifier}${pos}" id="${field.identifier}" onchange="insertSelectField('${remoteClass}', ${scriptName}, '${classname}','${field.identifier}', ${pos},'${field.identifier}Field${pos}',${patientId}, ${eventId},'${user}',${siteId},${formId})" style="visibility:hidden; display:none;">
        <option value=""> -- </option>
            <c:forEach var="enum" begin="0" items="${field.enumerations}">
                <c:if test='${enum.enabled ==true}'>
                    <c:choose>
