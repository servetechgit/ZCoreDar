...
function clearEditArea() {
    ...
    var editArea =  document.getElementById("ruleParamList");
	if (editArea) {
		editArea.style.display = "none";
    	editArea.style.visibility = "hidden";
	}
...
function dispatchPageItem(listField, id, event)
{
    if (listField == "fieldList") {
		...
		var adminInstructions = $("adminInstructions");
        ...
        if (calcTop >100) {
			...
			if(document.all) {
			    editArea.style.top = 120;
			} else {
				editArea.style.top = 110;
			}
        }
...
function dispatchCreateRules(link)
{
    ...
    if (pageItemId == "")
    {
		...
		// alert("pageItemId: "+ pageItemId);
        link.href += "?pageItem=" + pageItemId + "&providerClass=org.rti.zcore.FormField";
        return true;
    }
}
...
function displayPageItem(apageItem)
{
    ...
    DWRUtil.addOptions("field.type", [ 'Text','Enum','Integer','Long','Float','Date','Time','Year','Boolean','Yes/No','sex','MultiEnum','Display' ]);
...
function renderEnumInputField(fieldId) {
    ...
    var onChangeCreate = 'createEnumerations(\'createEnums\', ' + fieldId + ')';
    // input.setAttribute("onchange", onChangeCreate);
	input.onchange = function(){return createEnumerations('createEnums',fieldId)};
    // input.setAttribute("onclick", "alert('test)");
	// input.attachEvent("onchange", onChange);
...
function sortEnumerations(fieldId,formId,jsessionId) {
	// var id = fieldId.replace("field","");
	//alert("id" + id + " formId: " + formId);
	//alert("fieldId: " + fieldId + " formId: " + formId);
    window.location.href = "/dar/admin/enumeration/sort.do;jsessionid=" + jsessionId + "?formId=" + formId + "&fieldId=" + fieldId;
}
...
var getGlobalIdWidget = function(widgetLocation, name, value, column, id) {
	// flows = WidgetDisplay.dispatchFlows(initFlows);
	// alert("flows: " +flows)
	//DWRUtil.toDescriptiveString(flows, 3, 3)
	// alert(DWRUtil.toDescriptiveString(flows, 3, 3) );
	...
	DWRUtil.addOptions(select, ["Please select ..."]);
	if (value == "") {
		DWRUtil.addOptions(select, [ {name: 'Create Identifier', id: '1'}], "id", "name");
	}
	frag.appendChild(select);
	ele.innerHTML = "";
	ele.appendChild(frag);
...
// only for tables in the admin schema.
...
function displayFormUpdate(message)
{
	...
	Fat.fade_element(widget, 60, 2000, "#FFFF33", "#FFFFFF")
...
function fieldOrderChanged() {
    ...
    for (i = 0; i < newFieldOrderArray.length; i++) {
        ...
        if (pageItemList == "") {
        	...
        	pageItemList = pageItemList + "," + value;
        }
    }
    Dynasite.updateDisplayOrderList(confirmFieldOrderUpdate, pageItemList);
...
function fireMyPopup(div, hideDiv1) {
// kudos: http://dhtmlpopups.webarticles.org/usability.php

if (document.getElementById(div).style.display == "block") {
	document.getElementById(div).style.display = "none";
} else {
	// make sure hideDiv1 is hidden
	if (hideDiv1 != null)
