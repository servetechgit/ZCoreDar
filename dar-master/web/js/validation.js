...
function validateSqlField(form) {
    var isValid = true;
    var focusField = null;
    var i = 0;
    var fields = new Array();
	var searchField = 'new ' + retrieveFormName(form) + '_SqlFieldValidations()';
	// alert("searchField: " + searchField);
    oSqlField = eval(searchField);
    for (x in oSqlField) {
        var field = form[oSqlField[x][0]];
        if (! field) {     // check if it's a dwr widget
            var fieldString = oSqlField[x][0];
            var fieldId = fieldString.substring(5, fieldString.length);
            var fieldArray = document.getElementsByName(fieldString);
            field = fieldArray[0];
        }
        if (field) {
            if ((field.type == 'text' ||
                 field.type == 'textarea') &&
                (field.value.length > 0)) {
			var result = simpleValidateAlphanumeric(field.value);
                if (result == false) {
                    if (i == 0) {
                        focusField = field;
                    }
                    fields[i++] = oSqlField[x][1];
                    isValid = false;
                }
            }
        }
