...
images[0] = new Image();
images[1] = new Image();
images[0].src = "/dar/images/plus.gif";
images[1].src = "/dar/images/minus.gif";

/**
 * Toggles field and changes class of child to defaultClass.
 * Use
 * @param fieldType
 * @param valueToCompare - aka visibleEnumIdTrigger
 * @param childIdentifier - aka visibleDependency
 * @param parentIdentifier
 * @param notDefaultClass - if you do not want child object class changed to defaultCell
 * @return
 */
function toggleField(fieldType, valueToCompare, childIdentifier, parentIdentifier, notDefaultClass) {
	var cellType = "";
    if (fieldType == "dropdown")
    {
        var master = document.getElementById("select" + parentIdentifier);
        if (!master) {
            master = document.getElementById(parentIdentifier);
        }
    ...
    {
        var master = document.getElementsByName(parentIdentifier);
    ...
    {
        var master = document.getElementById(parentIdentifier);
        masterValue = master.value
    }
    else if (fieldType == "collapsing-display-header-begin")
    {
        var master = document.getElementById(parentIdentifier);
        masterValue = master.value
    }
    ...
    var lvals = childIdentifier.split(",");
    for (i = 0; i < lvals.length; i++)
    {
        var thisDependency = lvals[i];
        if (fieldType == "link")
        {
            var childDepName = thisDependency.substring(5);
        	var child = document.getElementById("tbl" + childDepName);
            var parent = child.parentNode;
            var buttonEl = "plusminus" + parentIdentifier;
            var button = document.getElementById(buttonEl);
            if (button) {
                button.src = (button.src == images[0].src)?images[1].src:images[0].src;
            }
            child.style.display = (child.style.display == "none")?"":"none";
            if (notDefaultClass == null) {
            	child.className = "defaultCell";
            }
        } else if (fieldType == "Yes/No Table") {
            ...
            if (! child) {
                child = document.getElementById("cell_" + thisDependency);
            }
            if (! child) {
            	child = document.getElementById(thisDependency);
            }
            if (! child) {
                child = document.getElementById("tbl" + thisDependency);
            }
			if (! child) {
                child = document.getElementById(thisDependency + "Cell0");
				cellType = "chart";
            }
            var testValue;
            ...
            //alert(" masterValue: " + masterValue + " testValue: " + testValue + " valueToCompare: " + valueToCompare);
            if (testValue == valueToCompare)
            {
                if (cellType == "chart") {
            ...
            if (masterValue == valueToCompare)
            {
                if (cellType == "chart") {
            		...
            		// alert("masterValue:" + masterValue + " visibleEnumIdTrigger: " + valueToCompare + " thisDependency: " + thisDependency + " child.style.display: " + child.style.display);
				}
            }
            else
            {
...
function toggleField2DepsChoice(theField,visibleEnumIdTrigger1,visibleDependency1, visibleEnumIdTrigger2,visibleDependency2, fieldid) {
    ...
    if (masterValue == visibleEnumIdTrigger1) {
     // reveal the fields we want
        ...
        // first loop through the fields we want to hide - visibleDependency2
        // if it doesn't match any of the fields we want to reveal (visibleDependency1), hide it
        for (i = 0; i < lvals2.length; i++)
        {
            var fieldToHide = lvals2[i];
			var hide = 'true';
            for (j = 0; j < lvals1.length; j++)
            {
                var fieldTodisplay = lvals1[j];
                if (fieldToHide == fieldTodisplay) {
          	       	hide = 'false';
                }
            }
			if (hide == 'true') {
				var child = document.getElementById("field" + fieldToHide);
                    if (! child) {
                        child = document.getElementById("tbl" + fieldToHide);
                    }
                    child.style.display = "none";
                    child.className = "defaultCell";
			}
    ...
    // reveal the fields we want
        ...
        // now hide the fields we don't want
		// first loop through the fields we want to hide - visibleDependency1
        // if it doesn't match any of the fields we want to reveal (visibleDependency2), hide it
        for (i = 0; i < lvals1.length; i++)
        {
            var fieldToHide = lvals1[i];
			var hide = 'true';
            for (j = 0; j < lvals2.length; j++)
            {
                var fieldTodisplay = lvals2[j];
                if (fieldToHide == fieldTodisplay) {
                 	hide = 'false';
                }
            }
...
/**
 * Only for selects
 * Display the field based on the order of the item chosen in the dropdown.
 * @param {Object} theField
 * @param {Object} visibleDependency1
 * @param {Object} fieldid
 */
function multitoggle(visibleDependency1, fieldid) {
    // only for selects
    ...
    masterOrder = master.selectedIndex -1;
	var lvals1 = visibleDependency1.split(",");
    // Loop  through the fields we want to see
    // if it doesn't match the masterOrder, hide it
    for (i = 0; i < lvals1.length; i++)
    {
	    var thisDependency = lvals1[i];
		var child = document.getElementById("cell" + thisDependency);
...
function validateNoSpaces(elementId) {
	var el = document.getElementById(elementId);
	var elValue = el.value;
	var result = simpleValidateNospaces(elValue);
   	if (result == false) {
   	 	alert("This field must not have any spaces.")
		el.value = "";
		setTimeout("document.getElementById('"+elementId+"').focus();",1);
		setTimeout("document.getElementById('"+elementId+"').select();",1);
   	}
...
function simpleValidateNospaces(value) {
    var isValid = true;
	// match whitespace
	var re = /\s/;
    if (value.length > 0) {
...
function simpleValidateAlphanumeric(value) {
    var isValid = true;
	// match any non-word characters (short for  [^a-zA-Z0-9_]).
	var re = /\W/;
    if (value.length > 0) {
        // remove '.' before checking digits
		if (value.match(re)) {
...
function toggleAgeCategory(manual) {
	var spanbirth_date = document.getElementById("spanbirth_date");
	var birth_date = document.getElementById("birth_date");
	//var birthdateDisplay = birth_date.innerHTML;
	var birthdateValue = birth_date.value;
	var age = document.getElementById("age_at_first_attendence");
	var ageCategory = document.getElementsByName("age_category");
	if (manual != null) {
		if (birthdateValue != "") {
		    var deleteYes = confirm("Changing the Age will delete the Birth Date. Proceed? ");
		    if (deleteYes) {
		    	birth_date.value = "";
		    	spanbirth_date.innerHTML = "";
		    } else {
		    	var dvals = birthdateValue.split("-");
		    	var year = dvals[0];
		    	var month =dvals[1];
		    	var day =dvals[2];
		    	dayString = new String(day);
		    	...
		    	var calcAgeValue = calcAge(dayString, monthString, year);
		    	// alert("Switching back to original value: " + calcAgeValue);
		    	age.value = calcAgeValue;
		    }
		}
	}
	if ((age.value != null) && (!isNaN(age.value))) {
		if (age.value > 14) {
			ageCategory[0].checked = true;
			ageCategory[1].checked = false;
		} else {
			ageCategory[0].checked = false;
			ageCategory[1].checked = true;
		}
	} else {
	}
}
...
function validateEmptyFields() {
	...
	var dispensedEmptyMessage = false;
	var itemEmptyMessage = false;
	var bothEmptyMessage = false;
	var PBF1_item_id = document.getElementById("PBF1_item_id");
	var PBF2_item_id = document.getElementById("PBF2_item_id");
	var PBF3_item_id = document.getElementById("PBF3_item_id");
	var PBF4_item_id = document.getElementById("PBF4_item_id");
	var PBF5_item_id = document.getElementById("PBF5_item_id");
	var PBF6_item_id = document.getElementById("PBF6_item_id");
	var PBF7_item_id = document.getElementById("PBF7_item_id");
	var PBF8_item_id = document.getElementById("PBF8_item_id");
	var PBF9_item_id = document.getElementById("PBF9_item_id");
	var PBF10_item_id = document.getElementById("PBF10_item_id");

	var PBF1_number_of_days = document.getElementById("PBF1_number_of_days");
	var PBF2_number_of_days = document.getElementById("PBF2_number_of_days");
	var PBF3_number_of_days = document.getElementById("PBF3_number_of_days");
	var PBF4_number_of_days = document.getElementById("PBF4_number_of_days");
	var PBF5_number_of_days = document.getElementById("PBF5_number_of_days");
	var PBF6_number_of_days = document.getElementById("PBF6_number_of_days");
	var PBF7_number_of_days = document.getElementById("PBF7_number_of_days");
	var PBF8_number_of_days = document.getElementById("PBF8_number_of_days");
	var PBF9_number_of_days = document.getElementById("PBF9_number_of_days");
	var PBF10_number_of_days = document.getElementById("PBF10_number_of_days");

	var PBF1_dispensed = document.getElementById("PBF1_dispensed");
	var PBF2_dispensed = document.getElementById("PBF2_dispensed");
	var PBF3_dispensed = document.getElementById("PBF3_dispensed");
	var PBF4_dispensed = document.getElementById("PBF4_dispensed");
	var PBF5_dispensed = document.getElementById("PBF5_dispensed");
	var PBF6_dispensed = document.getElementById("PBF6_dispensed");
	var PBF7_dispensed = document.getElementById("PBF7_dispensed");
	var PBF8_dispensed = document.getElementById("PBF8_dispensed");
	var PBF9_dispensed = document.getElementById("PBF9_dispensed");
	var PBF10_dispensed = document.getElementById("PBF10_dispensed");

	var PBF1_dispensedValue = PBF1_dispensed.value;
	var PBF2_dispensedValue = PBF2_dispensed.value;
	var PBF3_dispensedValue = PBF3_dispensed.value;
	var PBF4_dispensedValue = PBF4_dispensed.value;
	var PBF5_dispensedValue = PBF5_dispensed.value;
	var PBF6_dispensedValue = PBF6_dispensed.value;
	var PBF7_dispensedValue = PBF7_dispensed.value;
	var PBF8_dispensedValue = PBF8_dispensed.value;
	var PBF9_dispensedValue = PBF9_dispensed.value;
	var PBF10_dispensedValue = PBF10_dispensed.value;

	var PBF1_number_of_daysValue = PBF1_number_of_days.value;
	var PBF2_number_of_daysValue = PBF2_number_of_days.value;
	var PBF3_number_of_daysValue = PBF3_number_of_days.value;
	var PBF4_number_of_daysValue = PBF4_number_of_days.value;
	var PBF5_number_of_daysValue = PBF5_number_of_days.value;
	var PBF6_number_of_daysValue = PBF6_number_of_days.value;
	var PBF7_number_of_daysValue = PBF7_number_of_days.value;
	var PBF8_number_of_daysValue = PBF8_number_of_days.value;
	var PBF9_number_of_daysValue = PBF9_number_of_days.value;
	var PBF10_number_of_daysValue = PBF10_number_of_days.value;

	PBF1_item_idValue = PBF1_item_id[PBF1_item_id.selectedIndex].value;
	PBF2_item_idValue = PBF2_item_id[PBF2_item_id.selectedIndex].value;
	PBF3_item_idValue = PBF3_item_id[PBF3_item_id.selectedIndex].value;
	PBF4_item_idValue = PBF4_item_id[PBF4_item_id.selectedIndex].value;
	PBF5_item_idValue = PBF5_item_id[PBF5_item_id.selectedIndex].value;
	PBF6_item_idValue = PBF6_item_id[PBF6_item_id.selectedIndex].value;
	PBF7_item_idValue = PBF7_item_id[PBF7_item_id.selectedIndex].value;
	PBF8_item_idValue = PBF8_item_id[PBF8_item_id.selectedIndex].value;
	PBF9_item_idValue = PBF9_item_id[PBF9_item_id.selectedIndex].value;
	PBF10_item_idValue = PBF10_item_id[PBF10_item_id.selectedIndex].value;


	if ((PBF1_item_idValue != "") & (PBF1_dispensedValue == "")) {
		dispensedEmptyMessage = true;
		PBF1_dispensed.className = "highlightRow";
		PBF1_dispensed.focus();
		validateStatus = false;
	} else if ((PBF1_item_idValue == "") & (PBF1_dispensedValue != "")) {
		itemEmptyMessage = true;
		PBF1_item_id.className = "highlightRowNoAlign";
		PBF1_item_id.focus();
		validateStatus = false;
	} else if ((PBF1_number_of_daysValue != "") && ((PBF1_item_idValue = "") || (PBF1_dispensedValue == ""))) {
		bothEmptyMessage = true;
		validateStatus = false;
	}

	if ((PBF2_item_idValue != "") & (PBF2_dispensedValue == "")) {
		dispensedEmptyMessage = true;
		PBF2_dispensed.className = "highlightRow";
		PBF2_dispensed.focus();
		validateStatus = false;
	} else if ((PBF2_item_idValue == "") & (PBF2_dispensedValue != "")) {
		itemEmptyMessage = true;
		PBF2_item_id.className = "highlightRowNoAlign";
		PBF2_item_id.focus();
		validateStatus = false;
	} else if ((PBF2_number_of_daysValue != "") && ((PBF2_item_idValue = "") || (PBF2_dispensedValue == ""))) {
		bothEmptyMessage = true;
		validateStatus = false;
	}

	if ((PBF3_item_idValue != "") & (PBF3_dispensedValue == "")) {
		dispensedEmptyMessage = true;
		PBF3_dispensed.className = "highlightRow";
		PBF3_dispensed.focus();
		validateStatus = false;
	} else if ((PBF3_item_idValue == "") & (PBF3_dispensedValue != "")) {
		itemEmptyMessage = true;
		PBF3_item_id.className = "highlightRowNoAlign";
		PBF3_item_id.focus();
		validateStatus = false;
	} else if ((PBF3_number_of_daysValue != "") && ((PBF3_item_idValue = "") || (PBF3_dispensedValue == ""))) {
		bothEmptyMessage = true;
		validateStatus = false;
	}

	if ((PBF4_item_idValue != "") & (PBF4_dispensedValue == "")) {
		dispensedEmptyMessage = true;
		PBF4_dispensed.className = "highlightRow";
		PBF4_dispensed.focus();
		validateStatus = false;
	} else if ((PBF4_item_idValue == "") & (PBF4_dispensedValue != "")) {
		itemEmptyMessage = true;
		PBF4_item_id.className = "highlightRowNoAlign";
		PBF4_item_id.focus();
		validateStatus = false;
	} else if ((PBF4_number_of_daysValue != "") && ((PBF4_item_idValue = "") || (PBF4_dispensedValue == ""))) {
		bothEmptyMessage = true;
		validateStatus = false;
	}

	if ((PBF5_item_idValue != "") & (PBF5_dispensedValue == "")) {
		dispensedEmptyMessage = true;
		PBF5_dispensed.className = "highlightRow";
		PBF5_dispensed.focus();
		validateStatus = false;
	} else if ((PBF5_item_idValue == "") & (PBF5_dispensedValue != "")) {
		itemEmptyMessage = true;
		PBF5_item_id.className = "highlightRowNoAlign";
		PBF5_item_id.focus();
		validateStatus = false;
	} else if ((PBF5_number_of_daysValue != "") && ((PBF5_item_idValue = "") || (PBF5_dispensedValue == ""))) {
		bothEmptyMessage = true;
		validateStatus = false;
	}

	if ((PBF6_item_idValue != "") & (PBF6_dispensedValue == "")) {
		dispensedEmptyMessage = true;
		PBF6_dispensed.className = "highlightRow";
		PBF6_dispensed.focus();
		validateStatus = false;
	} else if ((PBF6_item_idValue == "") & (PBF6_dispensedValue != "")) {
		itemEmptyMessage = true;
		PBF6_item_id.className = "highlightRowNoAlign";
		PBF6_item_id.focus();
		validateStatus = false;
	} else if ((PBF6_number_of_daysValue != "") && ((PBF6_item_idValue = "") || (PBF6_dispensedValue == ""))) {
		bothEmptyMessage = true;
		validateStatus = false;
	}

	if ((PBF7_item_idValue != "") & (PBF7_dispensedValue == "")) {
		dispensedEmptyMessage = true;
		PBF7_dispensed.className = "highlightRow";
		PBF7_dispensed.focus();
		validateStatus = false;
	} else if ((PBF7_item_idValue == "") & (PBF7_dispensedValue != "")) {
		itemEmptyMessage = true;
		PBF7_item_id.className = "highlightRowNoAlign";
		PBF7_item_id.focus();
		validateStatus = false;
	} else if ((PBF7_number_of_daysValue != "") && ((PBF7_item_idValue = "") || (PBF7_dispensedValue == ""))) {
		bothEmptyMessage = true;
		validateStatus = false;
	}

	if ((PBF8_item_idValue != "") & (PBF8_dispensedValue == "")) {
		dispensedEmptyMessage = true;
		PBF8_dispensed.className = "highlightRow";
		PBF8_dispensed.focus();
		validateStatus = false;
	} else if ((PBF8_item_idValue == "") & (PBF8_dispensedValue != "")) {
		itemEmptyMessage = true;
		PBF8_item_id.className = "highlightRowNoAlign";
		PBF8_item_id.focus();
		validateStatus = false;
	} else if ((PBF8_number_of_daysValue != "") && ((PBF8_item_idValue = "") || (PBF8_dispensedValue == ""))) {
		bothEmptyMessage = true;
		validateStatus = false;
	}

	if ((PBF9_item_idValue != "") & (PBF9_dispensedValue == "")) {
		dispensedEmptyMessage = true;
		PBF9_dispensed.className = "highlightRow";
		PBF9_dispensed.focus();
		validateStatus = false;
	} else if ((PBF9_item_idValue == "") & (PBF9_dispensedValue != "")) {
		itemEmptyMessage = true;
		PBF9_item_id.className = "highlightRowNoAlign";
		PBF9_item_id.focus();
		validateStatus = false;
	} else if ((PBF9_number_of_daysValue != "") && ((PBF9_item_idValue = "") || (PBF9_dispensedValue == ""))) {
		bothEmptyMessage = true;
		validateStatus = false;
	}

	if ((PBF10_item_idValue != "") & (PBF10_dispensedValue == "")) {
		dispensedEmptyMessage = true;
		PBF10_dispensed.className = "highlightRow";
		PBF10_dispensed.focus();
		validateStatus = false;
	} else if ((PBF10_item_idValue == "") & (PBF10_dispensedValue != "")) {
		itemEmptyMessage = true;
		PBF10_item_id.className = "highlightRowNoAlign";
		PBF10_item_id.focus();
		validateStatus = false;
	} else if ((PBF10_number_of_daysValue != "") && ((PBF10_item_idValue = "") || (PBF10_dispensedValue == ""))) {
		bothEmptyMessage = true;
		validateStatus = false;
	}

	if (dispensedEmptyMessage == true) {
		alert("Dispensed value is empty. If you select an Item, you must enter a value for Quantity Dispensed.");

	}
	if (itemEmptyMessage == true) {
		alert("Item is empty. If you enter a value for Quantity Dispensed, you must select an Item");
	}

	if (bothEmptyMessage == true) {
		alert("You must select an Item and enter a value for Quantity Dispensed.");
	}

	return validateStatus;
}

function setDropdown(dropdownId,value) {
	var regimenDropdown = document.getElementById('regimen_id');
	//regimenDropdown.selectedIndex = value;
	//alert('selected: ' + value);
	
	 for (var i=0; i<regimenDropdown.options.length; i++) {  
		 if ( regimenDropdown.options[i].value == value )  {   
			 regimenDropdown.options[i].selected = true;   
			 isFound = true;  
			 } 
		 }
